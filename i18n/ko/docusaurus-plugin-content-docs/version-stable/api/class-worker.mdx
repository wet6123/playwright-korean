...
Worker 클래스는 [WebWorker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API)를 나타냅니다. `worker` 이벤트는 페이지 객체에서 작업자 생성을 알리기 위해 발생합니다. `close` 이벤트는 작업자가 사라질 때 작업자 객체에서 발생합니다.
...
[pageFunction](/api/class-worker.mdx#worker-evaluate-option-expression)의 반환 값을 반환합니다.

[worker.evaluate()](/api/class-worker.mdx#worker-evaluate)에 전달된 함수가 [Promise]를 반환하면, [worker.evaluate()](/api/class-worker.mdx#worker-evaluate)는 Promise가 resolve될 때까지 기다렸다가 해당 값을 반환합니다.

[worker.evaluate()](/api/class-worker.mdx#worker-evaluate)에 전달된 함수가 [Serializable]이 아닌 값을 반환하면, [worker.evaluate()](/api/class-worker.mdx#worker-evaluate)는 `undefined`를 반환합니다. Playwright는 또한 `JSON`으로 직렬화할 수 없는 몇 가지 추가 값(`-0`, `NaN`, `Infinity`, `-Infinity`)의 전송을 지원합니다.
...
- `arg` [EvaluationArgument] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="worker-evaluate-option-arg"/><a href="#worker-evaluate-option-arg" class="list-anchor">#</a>
  ...
  [pageFunction](/api/class-worker.mdx#worker-evaluate-option-expression)에 전달할 선택적 인수입니다.
...
[pageFunction](/api/class-worker.mdx#worker-evaluate-handle-option-expression)의 반환 값을 [JSHandle]로 반환합니다.

[worker.evaluate()](/api/class-worker.mdx#worker-evaluate)와 [worker.evaluateHandle()](/api/class-worker.mdx#worker-evaluate-handle)의 유일한 차이점은 [worker.evaluateHandle()](/api/class-worker.mdx#worker-evaluate-handle)이 [JSHandle]을 반환한다는 것입니다.

[worker.evaluateHandle()](/api/class-worker.mdx#worker-evaluate-handle)에 전달된 함수가 [Promise]를 반환하면, [worker.evaluateHandle()](/api/class-worker.mdx#worker-evaluate-handle)은 Promise가 resolve될 때까지 기다렸다가 해당 값을 반환합니다.
...
- `arg` [EvaluationArgument] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="worker-evaluate-handle-option-arg"/><a href="#worker-evaluate-handle-option-arg" class="list-anchor">#</a>
  ...
  [pageFunction](/api/class-worker.mdx#worker-evaluate-handle-option-expression)에 전달할 선택적 인수입니다.
...
이 전용 [WebWorker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API)가 종료될 때 발생합니다.
